...
import { bookingTransition, type DriverBooking } from "@/lib/driver/types";
import { updateDriverBooking } from "./actions";

export default function BookingControls({ booking }: { booking: DriverBooking }) {
  const [state, action, pending] = useActionState(updateDriverBooking, undefined);
  const [cancelling, setCancelling] = useState(false);
  const next = booking.status === "pending" ? "confirm" : booking.driver_status === "scheduled" ? "start" : "complete";
  const label = next === "confirm" ? "Confirm booking" : next === "start" ? "Start trip" : "Complete trip";
