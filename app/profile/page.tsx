  ...
  return <PageShell>
    <section className="mx-auto max-w-5xl px-5 py-12 sm:px-8 lg:py-16">
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="bg-gradient-to-r from-emerald-600 to-indigo-600 px-7 py-10 text-white sm:px-10">
        ...
        <div className="grid gap-5 p-7 sm:grid-cols-2 sm:p-10">
          ...
          <Link href="/wishlist" className="flex items-center justify-between rounded-2xl border border-emerald-200 p-5 transition hover:bg-emerald-50 sm:col-span-2">
      ...
      <section className="mt-10">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div><p className="text-sm font-bold uppercase tracking-[0.16em] text-emerald-600">Your trips</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Bookings and reservations</h2></div>
          {!reservationsError && reservations.length > 0 && <span className="rounded-full bg-emerald-50 px-3 py-1 text-sm font-semibold text-emerald-700">{reservations.length} {reservations.length === 1 ? "booking" : "bookings"}</span>}
        </div>
        ...
        {reservationsError ? (
          ...
          <div className="mt-5 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center"><CalendarDays className="mx-auto size-9 text-slate-400" /><h3 className="mt-4 font-bold">No bookings yet</h3><p className="mt-1 text-sm text-slate-500">Your confirmed stays, meals, and rides will appear here.</p><Link href="/airbnb" className="mt-5 inline-flex rounded-full bg-emerald-700 px-5 py-2.5 text-sm font-semibold text-white">Explore listings</Link></div>
        ) : (
              ...
              return <article key={reservation.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="grid sm:grid-cols-[190px_minmax(0,1fr)]">
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <Link href={`/reservations/${reservation.id}`} className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-emerald-700 hover:text-emerald-900">View confirmation <ChevronRight className="size-4" /></Link>
                  </div>
                </div>
              </article>;
            })}
