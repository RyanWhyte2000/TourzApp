...
export function BookingCard({ booking, type }: { booking: DashboardBooking; type: ProviderType }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-5">
    ...
    <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
      ...
      {type !== "restaurant_owner" && <p className="font-semibold">{money(booking.subtotal)} <span className="font-normal text-slate-400">service amount</span></p>}
    </div>
...
export default function ProviderDashboard({ profile, profiles = [profile], services, bookings, now }: { profile: ProviderProfile; profiles?: ProviderProfile[]; services: DashboardListing[]; bookings: DashboardBooking[]; now: number }) {
    ...
    <main className="min-w-0 bg-[#fafbfe] p-5 sm:p-8">
      ...
      {tab === "overview" && <>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
          ...
          ...(type === "restaurant_owner" ? [] : [{ label: "Booked service value", value: money(stats.bookedValue), icon: CircleDollarSign, detail: "All non-cancelled bookings; not payouts" }]),
