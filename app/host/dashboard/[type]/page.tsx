...
export default async function ProviderDashboardPage({ params }: { params: Promise<{ type: string }> }) {
  ...
  const { supabase, user, profile, profiles, error, loadedAt } = await getProviderAccount(type);
  ...
  return <PageShell showSearch={false}><ProviderDashboard profiles={profiles} profile={profile} services={(listingsResult.data ?? []) as DashboardListing[]} bookings={(bookingsResult.data ?? []) as unknown as DashboardBooking[]} now={loadedAt} /></PageShell>;
}
