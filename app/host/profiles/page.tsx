...
export default async function ProviderProfilesPage() {
  ...
  return <PageShell><main className="mx-auto max-w-5xl px-5 py-12 sm:px-8">
    ...
    <div className="mt-8 grid items-start gap-5 sm:grid-cols-2">
      {providerTypes.map((type) => <Link key={type} href={`/host/profiles/${type}`} className="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-emerald-400 hover:bg-emerald-50 focus-visible:outline-emerald-600">
        ...
        <span className="mt-5 block text-sm font-semibold text-emerald-700">Set up profile →</span>
      </Link>)}
    </div>
  </main></PageShell>;
}
