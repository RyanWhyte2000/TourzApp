...
export async function saveProviderProfile(_: ProviderState, form: FormData): Promise<ProviderState> {
  ...
  const { data: existing, error: lookupError } = await supabase.from("provider_profiles").select("id, provider_type").eq("user_id", user.id).eq("provider_type", provider_type).maybeSingle();
  ...
  revalidatePath("/host/dashboard", "layout");
