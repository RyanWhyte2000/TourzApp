...
export default async function ProviderProfilePage({ params }: { params: Promise<{ type: string }> }) {
  ...
  return <PageShell><main className="mx-auto max-w-5xl px-5 py-12 sm:px-8">
    <Link href={profile ? "/profile" : "/host/profiles"} className="text-sm font-semibold text-emerald-700">← {profile ? "Your account" : "Choose a different type"}</Link>
    <header className="my-8 flex flex-wrap items-center justify-between gap-5">
      ...
      {profile && <Link href="/host/onboarding" className="rounded-full bg-emerald-700 px-5 py-3 text-sm font-semibold text-white">{definition.listingLabel}</Link>}
    </header>
    {profile && type === "driver" && <Link href="/driver" className="mb-6 flex items-center justify-between rounded-2xl bg-emerald-700 p-5 font-semibold text-white"><span>Open your driver dashboard<span className="mt-1 block text-sm font-normal text-emerald-100">Manage services, bookings, and trip progress.</span></span><span aria-hidden="true">→</span></Link>}
