...
const inputClass = "mt-2 h-12 w-full rounded-xl border border-slate-200 bg-white px-4 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100";
...
export default function ProviderProfileForm({ type, profile }: { type: ProviderType; profile: ProviderProfile | null }) {
  ...
  return <form action={action} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    ...
    <button disabled={pending} className="mt-6 rounded-full bg-emerald-700 px-6 py-3 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-50">{pending ? "Saving…" : profile ? "Save profile" : "Create provider profile"}</button>
  </form>;
}
