...
export async function createHostListing(_: OnboardingState, formData: FormData): Promise<OnboardingState> {
  ...
  if (!isProviderType(providerType)) return { error: "Choose a provider category for this listing." };
  const { data: profile, error: profileError } = await supabase.from("provider_profiles").select("id, provider_type").eq("user_id", user.id).eq("provider_type", providerType).maybeSingle();
  ...
  const price = category === "food" ? 0 : Number(formData.get("price"));
  if (title.length < 5 || description.length < 30 || location.length < 3) return { error: "Complete the required listing details." };
  if (category !== "food" && (!Number.isFinite(price) || price <= 0 || price > 1_000_000)) return { error: "Enter a valid price." };
  ...
  const suffix: Record<ListingCategory, string> = { airbnb: "/night", hotel: "/night", food: "", transport: "/day" };
