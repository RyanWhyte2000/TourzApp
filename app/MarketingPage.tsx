    ...
    <PageShell>
      <div className="px-5 py-12 sm:px-8 sm:py-16 lg:px-14 lg:py-20">
        <section className="mx-auto max-w-3xl text-center">
          ...
          {action && (
            <Link href={action.href} className="mt-8 inline-flex rounded-full bg-emerald-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700">
              {action.label}
            </Link>
          )}
        </section>
        <section className="mx-auto mt-14 grid max-w-5xl gap-5 md:grid-cols-3">
          {features.map(({ icon: Icon, title: featureTitle, description: featureDescription }) => (
            <article key={featureTitle} className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
              <span className="flex size-11 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700"><Icon className="size-5" /></span>
